...
bool Record_Start(String audio_filename) 
{
  ...
  if (!flg_is_recording) { // entering 1st time -> create new file with unique name and WAV header
    flg_is_recording = true;

    // Generate a unique filename
    int file_index = 0;
    String unique_filename = audio_filename;
    while (SD.exists(unique_filename)) {
      file_index++;
      unique_filename = audio_filename + "_" + String(file_index) + ".wav"; // e.g., "audio_1.wav", "audio_2.wav"
    }

    File audio_file = SD.open(unique_filename, FILE_WRITE);
    if (audio_file) {
      audio_file.write((uint8_t *)&myWAV_Header, 44);
      audio_file.close();
      DebugPrintln("> WAV Header generated, Audio Recording started ... File: " + unique_filename);
      return true;
    } else {
      Serial.println("ERROR in Record_Start() - Failed to create audio file!");
      return false;
    }
  }
  ...
  if (flg_is_recording) { // here we land when recording started already -> task: append record buffer to file
    ...
    if (audio_file) {
      ...
      } else if (BITS_PER_SAMPLE == 8) { // 8-bit mode
